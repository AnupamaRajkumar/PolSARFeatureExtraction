...
Mat cvFeatures::GetStatistic(const Mat& src) {
    ...
    Mat output = Mat(1, 5, CV_64FC1);

        vector<double> vec;
        auto* p = src.data;
        int size = static_cast<int>(src.total())*src.channels();
        ...
        {
            //median
            output.at<double>(0, 0)= vec[size / 2 - 1] + vec[size / 2] / 2.0;
        }
        else
        {
            //median
            output.at<double>(0, 0) = vec[size / 2];
        }

        //min
        output.at<double>(0, 1) = vec[0];
        //max
        output.at<double>(0, 2) = vec[size - 1];

        Scalar mean, stddev; //0:1st channel, 1:2nd channel and 2:3rd channel
        meanStdDev(vec, mean, stddev);
        output.at<double>(0, 3) = mean[0];
        output.at<double>(0, 4) = stddev[0];
