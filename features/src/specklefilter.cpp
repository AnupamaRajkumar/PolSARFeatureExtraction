...
void RefinedLee::filterFullPol(Mat& hh,  Mat& vv,  Mat& hv) {
    ...
    Mat span = Mat(Size(hh.size()), CV_64FC1);
...
void RefinedLee::filterFullPol(Mat& hh, Mat& vv, Mat& hv, Mat&span) {
    ...
    Mat neighborPixelValues = Mat(filterSize, filterSize, CV_64FC1);
    Mat neighborSpanValues = Mat(filterSize, filterSize, CV_64FC1);
    ...
    for (int i = 0; i < 6; i++) {
        Mat t = temp.at(i);
        Mat temp_output = Mat(Size(hh.size()), CV_64FC1);
        for (int x = 0; x < hh.rows; x++) {
            for (int y = 0; y < hh.cols; y++) {
                ...
                if (n < filterSize * filterSize) {
                    temp_output.at<double>(x, y) = computePixelValueUsingLocalStatistics(neighborPixelValues, n);
                }
                else {
                    temp_output.at<double>(x, y) = computePixelValueUsingEdgeDetection(neighborPixelValues, neighborSpanValues);
                }
            }
        }
...
/**
     ...
     */
double RefinedLee::computePixelValueUsingLocalStatistics(const Mat& neighborPixelValues, int numSamples) {
    ...
    double meanY = getLocalMeanValue(neighborPixelValues, numSamples);
    double varY = getLocalVarianceValue(neighborPixelValues, numSamples,meanY);
    if (varY == 0.0) {
        return 0.0;
    }
    ...
    double b = varX / varY;
    return meanY + b * (neighborPixelValues.at<double>(halfFilterSize,halfFilterSize) - meanY);
}
 ...
 * @return The filtered pixel value.
 */
double RefinedLee::computePixelValueUsingEdgeDetection(const Mat & neighborPixelValues, const Mat& neighborSpanValues) {
    
    int halfFilterSize = filterSize / 2;

    Mat subAreaMeans = Mat(3, 3, CV_64FC1);
    ...
    double meanY = getLocalMeanValue(spanPixels, numPixels);
    double varY = getLocalVarianceValue(spanPixels, numPixels,meanY);
    if (varY == 0.0) {
        return 0.0;
    }
    ...
    double meanZ = getLocalMeanValue(elemPixels, numPixels);

    return meanZ + b * (neighborPixelValues.at<double>(halfFilterSize,halfFilterSize) - meanZ);
}
    ...
    for (int j = 0; j < filterSize; ++j) {
        ...
        if (yj < 0 || yj >= maxY) {
            for (int i = 0; i < filterSize; ++i) {
                neighborPixelValues.at<double>(i, j) = NonValidPixelValue;
                neighborSpanValues.at<double>(i, j) = NonValidPixelValue;
            }
            continue;
        }
        for (int i = 0; i < filterSize; ++i) {
            ...
            if (xi < 0 || xi >= maxY) {
                ...
                neighborPixelValues.at<double>(i, j) = src.at<double>(xi, yj);
                neighborSpanValues.at<double>(i, j) = span.at<double>(xi, yj);
                numSamples++;
            }
        }
    }
...
**/
double RefinedLee::getLocalMeanValue(const Mat & neighborValues, int numSamples) {

        double mean = 0.0;
        for (int i = 0; i < neighborValues.rows; i++){
            for (int j = 0; j < neighborValues.cols; j++){
                double v = neighborValues.at<double>(i, j);
     ...
     */
 double RefinedLee::getLocalVarianceValue(const Mat& neighborValues, int numSamples, double mean) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int y = y0; y < y0 + subWindowSize; y++) {
                for (int x = x0; x < x0 + subWindowSize; x++) {
                    mean += neighborPixelValues.at<double>(y,x);
                }
            }
            subAreaMeans.at<double>(j,i) = mean / subWindowSizeSqr;
        }
    }
}
...
/**
 ...
 int RefinedLee::getDirection(const Mat & subAreaMeans) {
    ...
    // horizontal gradient
    gradient[0] = subAreaMeans.at<double>(0,2) + subAreaMeans.at<double>(1,2) + subAreaMeans.at<double>(2,2) -
        subAreaMeans.at<double>(0,0) - subAreaMeans.at<double>(1,0) - subAreaMeans.at<double>(2,0);

    gradient[1] = subAreaMeans.at<double>(0,1) + subAreaMeans.at <double>(0,2) + subAreaMeans.at<double>(1,2) -
        subAreaMeans.at<double>(1,0)- subAreaMeans.at <double>(2,0) - subAreaMeans.at<double>(2,1);

    gradient[2] = subAreaMeans.at<double>(0,0) + subAreaMeans.at<double>(0,1) + subAreaMeans.at<double>(0,2) -
        subAreaMeans.at<double>(2,0)- subAreaMeans.at<double>(2,1) - subAreaMeans.at<double>(2,2);

    gradient[3] = subAreaMeans.at<double>(0,0) + subAreaMeans.at<double>(0,1) + subAreaMeans.at < double>(1,0) -
        subAreaMeans.at<double>(1,2) - subAreaMeans.at<double>(2,1) - subAreaMeans.at<double>(2,2);
 ...
 int RefinedLee::getNonEdgeAreaPixelValues(const Mat& neighborPixelValues, int d, Mat &pixels) {
     ...
     int halfFilterSize = filterSize / 2;
     pixels = Mat::zeros(filterSize, filterSize, CV_64FC1);
     int k = 0;
     ...
     case 0: {
         ...
         for (int y = 0; y < filterSize; y++) {
             for (int x = halfFilterSize; x < filterSize; x++) {
                 pixels.at<double>(y, x)= neighborPixelValues.at<double>(y, x);
                 k++;
             }
         }
         break;
